...
offset=90;   % multi-offset &the maximum offset (floor(1.2*nz*dx))
offmin=6;    % no need to change
...
parfor is=1:ns
    ...
    [seismo_v_d1]=weight_dataAD(seismo_v,dt,dx,df,offset,sx(is),gx,w,fmin,fmax, ...
    cr_0(:,is),cr_0l(:,is),cr_pre_r(:,is),cr_pre_l(:,is));% Uncomment there two lines to enable the WD method
%     [seismo_v_d1]=ADWDgrad_w(nt,ng,npair,w,SoftArgNorm,grad_outputr,grad_outputl,space_M,saveForBackwardr,saveForBackwardl);% Uncomment this line to enable the SWD method
