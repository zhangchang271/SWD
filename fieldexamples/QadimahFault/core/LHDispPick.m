
function [res,cr_pre] = LHDispPick(ml,npair,vmin,cr_0,saveForBackward,method)
mode=1;
if isfield(saveForBackward,'uxtposr')
    offset = length(saveForBackward.uxtposr);
    [~,ini] = max(max(saveForBackward.mlr));
else
    offset = length(saveForBackward.uxtposl);
    [~,ini] = max(max(saveForBackward.mll));
end
if offset>100
    [~,line_start_stop,line_part]=pickdisp_xl(ml);
    [line_start_stop,line_part]=CEDC(ml);
else
    [~,p_max] = max(ml);
    llp = length(p_max);
    line_part{1}=[p_max;1:llp];
    line_start_stop=[1,llp];
end

if mode==1
    if method==1
    [idx,~,~]=pickFDC1(ml,ini,2,4);
    cr_pre = idx' + vmin;
    res = mean(((cr_pre(50:end-30,1) - cr_0(50:end-30,1)).^2));
    else
    cr_pre = 1.*ones(npair,1);
    cr_pre(line_start_stop(mode,1):line_start_stop(mode,2),1) = line_part{mode}(1,:)+vmin;
    res = mean(((cr_pre - cr_0).^2));
    end
else 
    if length(line_part)>=mode
            cr_pre = 1.*ones(npair,1);
            cr_pre(line_start_stop(mode,1):line_start_stop(mode,2),1) = line_part{mode}(1,:)+vmin;
            res = mean(((cr_pre - cr_0).^2));
    else
        cr_pre = 1.*ones(npair,1);
        res = mean(((cr_pre - cr_0).^2));
    end
end
    
end